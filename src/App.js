...
const showNotification = (title, body, icon="❤️") => {
...
/* ─── DESIGN ─── */
const THEMES = {
  rose:     { name:"로즈 크림",   p:"#C4536A", d:"#9E3650", l:"#F9EEF0", pale:"#FBF7F8", sub:"#9C8088", border:"rgba(180,130,140,.15)", card:"#FFFFFF", nav:"#FFFFFF", text:"#1C1016", online:"#6DBF8A" },
  ivory:    { name:"아이보리",    p:"#A08060", d:"#7A6040", l:"#F5EFE6", pale:"#FAF7F2", sub:"#9C8870", border:"rgba(160,128,96,.14)", card:"#FFFFFF", nav:"#FFFFFF", text:"#1C1810", online:"#6DBF8A" },
  blush:    { name:"블러쉬",      p:"#C06080", d:"#904060", l:"#FCF0F3", pale:"#FDF8F9", sub:"#9C7888", border:"rgba(192,96,128,.12)", card:"#FFFFFF", nav:"#FFFFFF", text:"#1C1018", online:"#6DBF8A" },
  sage:     { name:"세이지 크림", p:"#607A60", d:"#405A40", l:"#EEF3EE", pale:"#F6FAF6", sub:"#788878", border:"rgba(96,122,96,.14)", card:"#FFFFFF", nav:"#FFFFFF", text:"#101C10", online:"#6DBF8A" },
  midnight: { name:"미드나잇",    p:"#D4748A", d:"#B05468", l:"#2A1820", pale:"#160E12", sub:"#A09098", border:"rgba(212,116,138,.18)", card:"#221828", nav:"#160E12", text:"#F0E8EA", online:"#6DBF8A" },
};
...
export default function App() {
  ...
  const [eventForm,setEventForm] = useState({title:"",color:"#C4536A"});
  ...
  const GS = `
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; -webkit-tap-highlight-color:transparent; }
    ...
    @keyframes scaleIn { from{opacity:0;transform:scale(.96) translateY(14px)} to{opacity:1;transform:scale(1) translateY(0)} }
    @keyframes spin { to{transform:rotate(360deg)} }
    @keyframes hb { 0%,100%{transform:scale(1)} 14%{transform:scale(1.12)} 28%{transform:scale(1)} 42%{transform:scale(1.08)} 70%{transform:scale(1)} }
    ...
    .hb  { animation:hb 2.4s ease-in-out infinite; display:inline-block; }
    .spin{ animation:spin 1s linear infinite; display:inline-block; }
    .tab-active { animation:bounce .3s ease; }
    .fu1{animation-delay:.06s} .fu2{animation-delay:.12s} .fu3{animation-delay:.18s}
  `;

  const card  = { background:T.card, borderRadius:20, padding:"18px 20px", marginBottom:12, boxShadow:`0 1px 0 ${T.border},0 4px 24px rgba(0,0,0,.05)`, border:`1px solid ${T.border}` };
  const btnPrimary = (w="100%") => ({ background:`linear-gradient(135deg,${T.p} 0%,${T.d} 100%)`, color:"#fff", border:"none", borderRadius:14, padding:"15px 22px", fontWeight:600, fontSize:15, cursor:"pointer", width:w, fontFamily:"'Noto Sans KR',sans-serif", boxShadow:`0 4px 18px ${T.p}40` });
  const btnSoft    = (w="100%") => ({ background:T.l, color:T.p, border:"none", borderRadius:14, padding:"14px 20px", fontWeight:600, fontSize:14, cursor:"pointer", width:w, fontFamily:"'Noto Sans KR',sans-serif" });
  const btnOutline = (w="100%") => ({ background:"transparent", color:T.sub, border:`1px solid ${T.border}`, borderRadius:14, padding:"13px 20px", fontWeight:500, fontSize:14, cursor:"pointer", width:w, fontFamily:"'Noto Sans KR',sans-serif" });
  ...
  const pill  = (active) => ({ background:active?T.p:T.l, color:active?"#fff":T.p, border:"none", borderRadius:50, padding:"8px 18px", fontWeight:500, fontSize:13, cursor:"pointer", flexShrink:0, fontFamily:"'Noto Sans KR',sans-serif", transition:"all .2s" });
  ...
  useEffect(() => {
    ...
    return onSnapshot(doc(db,"presence",couple.partnerId), snap => {
      ...
      const online = d.online && last && (new Date()-last)<65000;
      if (online && !partnerOnline && notifEnabled) showNotification(`${couple.partnerName}이 접속했어요`, "지금 채팅을 시작해보세요 💕");
  ...
  useEffect(() => {
    if (messages.length > prevMsgCount && prevMsgCount > 0) {
      ...
      if (last?.from !== user?.uid && tab !== "chat" && notifEnabled) {
        showNotification(`${last.fromName}`, last.type==="image"?"사진을 보냈어요 📸":last.text);
        setUnread(u => u+1);
      }
    }
  ...
  const onAvatarUpload = async e => {
    ...
    try{const url=await uploadImg(file);await updateDoc(doc(db,"users",user.uid),{photoURL:url});setMyAvatar(url);}
    catch(err){alert(err.message);}finally{setUploading(false);}
  };
  const onBgUpload = async e => {
    const file=e.target.files[0]; if(!file||!couple) return; setUploading(true);
    try{const url=await uploadImg(file);await updateDoc(doc(db,"couples",couple.coupleId),{sharedBg:url});setSharedBg(url);}
    catch(err){alert(err.message);}finally{setUploading(false);}
  };
  ...
  const onChatPhoto = async e => {
    const file=e.target.files[0]; if(!file) return; setUploading(true);
    try{const url=await uploadImg(file);await addDoc(collection(db,"couples",couple.coupleId,"messages"),{text:url,from:user.uid,fromName:myName,type:"image",createdAt:serverTimestamp()});}
    catch(err){alert(err.message);}finally{setUploading(false);}
  };
  ...
  const toggleMission = async i => {
    ...
    if(next.length===3&&notifEnabled) showNotification("🎉 오늘 미션 완료!", "두 분 수고하셨어요!");
  };

  const enableNotification = async () => {
    const ok = await requestNotificationPermission();
    setNotifEnabled(ok);
    if (!ok) alert("알림 설정에서 허용해주세요.");
    else showNotification("알림 설정 완료!", "채팅·접속·미션 알림을 받을게요 💕");
  };
  ...
  /* ════ LOADING ════ */
  if (loading) return (
    <div style={{maxWidth:430,margin:"0 auto",height:"100%",display:"flex",alignItems:"center",justifyContent:"center",background:THEMES.rose.pale}}>
      <style>{`@keyframes hb{0%,100%{transform:scale(1)}14%{transform:scale(1.12)}28%{transform:scale(1)}42%{transform:scale(1.08)}70%{transform:scale(1)}} .hb{animation:hb 2.4s ease-in-out infinite;display:inline-block;}`}</style>
  ...
  if (!user) return (
    <div style={{maxWidth:430,margin:"0 auto",height:"100%",position:"relative",overflow:"hidden",background:T.pale,display:"flex",flexDirection:"column",justifyContent:"flex-end",padding:"0 28px 56px"}}>
      <style>{GS}</style>
      <div style={{position:"absolute",top:0,left:0,right:0,height:"56%",background:`linear-gradient(160deg,${T.l} 0%,${T.pale} 100%)`,borderRadius:"0 0 64px 64px"}}/>
      <div style={{position:"absolute",top:60,left:0,right:0,display:"flex",flexDirection:"column",alignItems:"center",padding:"0 32px"}}>
        <div className="hb" style={{fontSize:68,marginBottom:24}}>❤️</div>
        <h1 className="fu serif" style={{fontSize:56,fontWeight:300,color:T.text,letterSpacing:"-.03em",lineHeight:1,textAlign:"center",marginBottom:10}}>우리</h1>
        <p className="fu fu1" style={{fontSize:16,color:T.sub,fontWeight:300,letterSpacing:".04em"}}>우리만의 공간</p>
      </div>
      <div className="fu fu2" style={{position:"relative",display:"flex",flexDirection:"column",gap:12}}>
        <button onClick={loginGoogle} style={{...btnPrimary(),borderRadius:18,fontSize:16,padding:"17px 24px",display:"flex",alignItems:"center",justifyContent:"center",gap:12}}>
        ...
        <button style={{...btnOutline(),borderRadius:18,fontSize:15,padding:"16px",opacity:.4,cursor:"not-allowed"}}>카카오로 시작하기 (준비중)</button>
      </div>
  ...
  if (showConnect) return (
    <div style={{maxWidth:430,margin:"0 auto",height:"100%",background:T.pale,display:"flex",flexDirection:"column",padding:"60px 24px 40px",overflow:"auto"}}>
      ...
      <div className="fu fu1" style={{background:T.l,borderRadius:20,padding:"18px 22px",marginBottom:18,border:`1px solid ${T.border}`}}>
        <p style={{fontSize:11,color:T.sub,fontWeight:600,letterSpacing:".06em",textTransform:"uppercase",marginBottom:10}}>내 코드</p>
        <p className="serif" style={{fontSize:36,fontWeight:500,color:T.p,letterSpacing:"10px",lineHeight:1}}>{myCode}</p>
      </div>
  ...
  return (
    <div style={{maxWidth:430,margin:"0 auto",height:"100%",background:T.pale,display:"flex",flexDirection:"column",overflow:"hidden",position:"relative"}}>
      ...
      <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",paddingBottom:80}}>
        ...
        {tab==="home" && (
          <div className="fi">
            <div style={{position:"relative",borderRadius:"0 0 32px 32px",overflow:"hidden",marginBottom:4}}>
              <div style={{background:sharedBg?`url(${sharedBg}) center/cover`:`linear-gradient(145deg,${T.p} 0%,${T.d} 100%)`,padding:"60px 24px 36px",minHeight:270,display:"flex",flexDirection:"column",justifyContent:"flex-end"}}>
                {sharedBg&&<div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom,rgba(0,0,0,.08) 0%,rgba(0,0,0,.52) 100%)"}}/>}
                <div style={{position:"absolute",top:54,left:24,right:24,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <button onClick={()=>setTab("settings")} style={{background:"rgba(255,255,255,.16)",border:"1px solid rgba(255,255,255,.22)",borderRadius:10,padding:"6px 12px",color:"rgba(255,255,255,.88)",fontSize:12,cursor:"pointer",backdropFilter:"blur(8px)",fontFamily:"'Noto Sans KR',sans-serif",fontWeight:500}}>⚙️ 설정</button>
                </div>
                <div style={{position:"relative"}}>
                  <p style={{fontSize:12,color:"rgba(255,255,255,.7)",marginBottom:4,letterSpacing:".02em"}}>{couple?"함께한 날":"오늘부터"}</p>
                  <p className="serif" style={{fontSize:74,fontWeight:300,color:"#fff",lineHeight:.88,letterSpacing:"-4px",marginBottom:10}}>D+{dday()}</p>
                  <p style={{fontSize:12,color:"rgba(255,255,255,.6)"}}>{couple?.startDate||todayStr()}</p>
                </div>
              </div>
            </div>

            {!couple && <div onClick={()=>setShowConnect(true)} style={{margin:"16px 20px 0",background:T.l,borderRadius:18,padding:"16px 20px",border:`1.5px dashed ${T.p}55`,cursor:"pointer"}}><p style={{fontWeight:600,color:T.p,marginBottom:2,fontSize:15}}>파트너와 연결하기</p><p style={{fontSize:13,color:T.sub}}>코드를 교환해서 모든 기능을 함께 써요</p></div>}
              ...
              {[{l:"100일",t:100},{l:"200일",t:200},{l:"1주년",t:365},{l:"2주년",t:730}]
                ...
                .map((a,i)=>(
                  <div key={i} style={{background:T.card,borderRadius:16,padding:"10px 16px",border:`1px solid ${T.border}`,flexShrink:0,textAlign:"center",minWidth:76}}>
                    ...
                    <p className="serif" style={{fontWeight:500,color:T.p,fontSize:16,letterSpacing:"-.02em"}}>D-{a.d}</p>
                  </div>
                ))}
            </div>}
              ...
              <div style={{...card,padding:"18px 20px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <Av src={diaries[0].who===user.uid?myAvatar:partnerAvatar} name={diaries[0].whoName} size={28}/>
                    <span style={{fontSize:13,color:T.text,fontWeight:500}}>{diaries[0].whoName}</span>
                    <span style={{fontSize:12,color:T.sub}}>{fmtDate(diaries[0].dateKey)}</span>
                  </div>
                  <span style={{fontSize:18}}>{diaries[0].mood}</span>
                </div>
                {diaries[0].photoUrl&&<img src={diaries[0].photoUrl} alt="" style={{width:"100%",borderRadius:12,marginBottom:10,maxHeight:150,objectFit:"cover"}}/>}
        ...
        {tab==="calendar" && (
          <div className="fi">
            <div style={{padding:"56px 20px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <h1 className="serif" style={{fontSize:32,fontWeight:400,color:T.text}}>달력</h1>
            ...
            <div style={{...card,margin:"0 16px 14px",padding:"18px 14px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18,padding:"0 2px"}}>
                ...
                <p className="serif" style={{fontSize:22,fontWeight:400,color:T.text,letterSpacing:"-.02em"}}>{calDate.getFullYear()}년 {calDate.getMonth()+1}월</p>
                <button onClick={()=>setCalDate(d=>new Date(d.getFullYear(),d.getMonth()+1,1))} style={{background:T.l,border:"none",width:38,height:38,borderRadius:12,cursor:"pointer",fontSize:18,color:T.p,display:"flex",alignItems:"center",justifyContent:"center"}}>›</button>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",marginBottom:8}}>
                {WEEK.map((d,i)=><div key={d} style={{textAlign:"center",fontSize:11,fontWeight:700,color:i===0?"#C45353":i===6?"#5370C4":T.sub,padding:"4px 0",letterSpacing:".02em"}}>{d}</div>)}
              </div>
                ...
                {calDays().map((day,i)=>{
                  ...
                  return (
                    <div key={i} onClick={()=>{setSelDay(day);setModal("dayDetail");}}
                      ...
                      {/* Background photo */}
                      {mainPhoto&&!isToday&&<img src={mainPhoto.url} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:.5,borderRadius:12}}/>}
                      {/* Soft bg if has events */}
                      {!mainPhoto&&(evs.length>0||hasDiary)&&!isToday&&<div style={{position:"absolute",inset:0,background:T.l,borderRadius:12}}/>}
                      {/* Day number */}
                      <span style={{fontSize:13,fontWeight:isToday?800:600,color:isToday?"#fff":mainPhoto?"#fff":dow===0?"#C45353":dow===6?"#5370C4":T.text,position:"relative",zIndex:1,textShadow:mainPhoto&&!isToday?"0 1px 4px rgba(0,0,0,.5)":"none"}}>{day}</span>
                      {/* Event titles (show first event title on bigger cells) */}
                      {evs.length>0&&!isToday&&!mainPhoto&&(
                        <span style={{fontSize:8,color:evs[0].color,fontWeight:700,position:"absolute",bottom:10,zIndex:1,maxWidth:"90%",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",letterSpacing:"-.01em"}}>{evs[0].title}</span>
                      )}
                      {/* Dot indicators */}
                      <div style={{display:"flex",gap:2,position:"absolute",bottom:3,zIndex:2}}>
                        {evs.slice(0,2).map((ev,ei)=><div key={ei} style={{width:4,height:4,borderRadius:"50%",background:isToday?"rgba(255,255,255,.8)":mainPhoto?"rgba(255,255,255,.9)":ev.color}}/>)}
                        {hasDiary&&<div style={{width:4,height:4,borderRadius:"50%",background:isToday||mainPhoto?"rgba(255,255,255,.8)":"#B89050"}}/>}
                      </div>
                    </div>
                  );
                })}
            ...
            <div style={{padding:"0 20px"}}>
              ...
              {Object.entries(events)
                ...
                .map((ev,i)=>(
                  <div key={i} style={{...card,padding:"13px 18px",marginBottom:8,display:"flex",alignItems:"center",gap:14}}>
                    ...
                    {ev.who===user.uid&&<div style={{display:"flex",gap:6}}>
                      ...
                      <button onClick={()=>delEvent(ev)} style={{background:"#FEE8EC",border:"none",borderRadius:8,width:30,height:30,cursor:"pointer",color:"#C45353",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
                    </div>}
                  </div>
                ))}
            </div>
          ...
          <div className="fi" style={{display:"flex",flexDirection:"column",height:"100%",position:"absolute",inset:0,paddingBottom:80}}>
            <div style={{background:T.card,padding:"54px 20px 14px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:14,flexShrink:0}}>
              <div style={{position:"relative"}}>
                ...
                <div style={{position:"absolute",bottom:1,right:1,width:11,height:11,borderRadius:"50%",background:couple&&partnerOnline?T.online:T.border,border:`2px solid ${T.card}`}}/>
              </div>
              <div>
                <p style={{fontWeight:600,fontSize:16,color:T.text}}>{couple?.partnerName||"파트너"}</p>
                <p style={{fontSize:12,color:couple&&partnerOnline?T.online:T.sub,fontWeight:500,marginTop:2}}>{couple?(partnerOnline?"지금 접속 중":"오프라인"):"파트너 미연결"}</p>
              </div>
            </div>
            {!couple ? (
        ...
        {tab==="photos" && (
          <div className="fi">
            <div style={{padding:"56px 20px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <h1 className="serif" style={{fontSize:32,fontWeight:400,color:T.text}}>사진첩</h1>
            ...
            {photos.filter(p=>p.albumId===selAlbum).length===0?(
              <div style={{textAlign:"center",padding:"80px 20px"}}>
                ...
                <button onClick={()=>setModal("photoUpload")} style={{...btnPrimary("auto"),padding:"13px 28px",borderRadius:16,marginTop:8}}>사진 추가하기</button>
              </div>
            ):(
              <div style={{padding:"0 20px"}}>
        ...
        {tab==="diary" && (
          <div className="fi">
            <div style={{padding:"56px 20px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <h1 className="serif" style={{fontSize:32,fontWeight:400,color:T.text}}>일기</h1>
            ...
            {couple&&<div style={{padding:"0 20px 14px",display:"flex",gap:8}}>
                ...
                {diaries.filter(d=>diaryFilter==="all"||(diaryFilter==="me"?d.who===user.uid:d.who!==user.uid)).map(d=>(
                  <div key={d.id} style={{...card,padding:"18px 20px",marginBottom:12}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                      <div style={{display:"flex",alignItems:"center",gap:10}}>
                        <Av src={d.who===user.uid?myAvatar:partnerAvatar} name={d.whoName} size={32}/>
                        <div><p style={{fontWeight:600,fontSize:14,color:T.text}}>{d.whoName}</p><p style={{fontSize:12,color:T.sub,marginTop:1}}>{fmtDate(d.dateKey)}</p></div>
                      </div>
                      <div style={{display:"flex",alignItems:"center",gap:6}}>
                        <span style={{fontSize:20}}>{d.mood}</span>
                        {d.who===user.uid&&(<>
                          <button onClick={()=>{setEditDiary(d);setDiaryForm({title:d.title||"",content:d.content,mood:d.mood,sticker:d.sticker||"",photoUrl:d.photoUrl||""});setModal("addDiary");}} style={{background:T.l,border:"none",borderRadius:8,width:30,height:30,cursor:"pointer",color:T.p,display:"flex",alignItems:"center",justifyContent:"center"}}>✎</button>
                          <button onClick={()=>delDiary(d)} style={{background:"#FEE8EC",border:"none",borderRadius:8,width:30,height:30,cursor:"pointer",color:"#C45353",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
                        </>)}
                      </div>
                    </div>
        ...
        {tab==="settings" && (
          <div className="fi">
            <div style={{padding:"56px 20px 14px"}}>
              <h1 className="serif" style={{fontSize:32,fontWeight:400,color:T.text}}>설정</h1>
            </div>
            <div style={{padding:"0 20px 14px",display:"flex",gap:8,overflowX:"auto",scrollbarWidth:"none"}}>
              {[{id:"profile",l:"프로필"},{id:"notify",l:"알림"},{id:"theme",l:"테마"},{id:"home",l:"홈 꾸미기"},{id:"mission",l:"미션"},{id:"couple",l:"커플"}].map(s=>(
            ...
            {settingsTab==="profile" && <div style={{padding:"0 20px"}}>
              <div style={{...card,padding:"22px 20px"}}>
                <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:18}}>
                  ...
                  <div>
                    <p style={{fontWeight:700,fontSize:20,color:T.text}}>{myName}</p>
            ...
            {settingsTab==="notify" && <div style={{padding:"0 20px"}}>
              <div style={{...card,padding:"22px 20px"}}>
                <p style={{fontSize:11,color:T.sub,fontWeight:600,letterSpacing:".06em",textTransform:"uppercase",marginBottom:16}}>알림 설정</p>
                <div style={{background:T.l,borderRadius:16,padding:"16px 18px",marginBottom:14}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div>
                      <p style={{fontWeight:600,fontSize:15,color:T.text,marginBottom:4}}>브라우저 알림</p>
                      <p style={{fontSize:13,color:T.sub}}>채팅, 접속 알림을 받아요</p>
                    </div>
                    <div style={{width:44,height:26,borderRadius:13,background:notifEnabled?T.p:T.border,position:"relative",cursor:"pointer",transition:"background .2s"}} onClick={notifEnabled?()=>{}:enableNotification}>
                      <div style={{width:20,height:20,borderRadius:"50%",background:"#fff",position:"absolute",top:3,left:notifEnabled?21:3,transition:"left .2s",boxShadow:"0 1px 4px rgba(0,0,0,.2)"}}/>
                    </div>
                  </div>
                </div>
                {!notifEnabled && <div style={{background:"#FFF8EC",borderRadius:14,padding:"14px 16px",border:"1px solid #F0D080"}}>
                  <p style={{fontSize:13,color:"#8A6010",lineHeight:1.6}}>
                    <strong>알림 설정 방법</strong><br/>
                    아이폰: Safari → 주소창 왼쪽 "AA" → 웹 사이트 설정 → 알림 허용<br/><br/>
                    버튼을 누르면 허용 여부를 묻는 팝업이 나타나요
                  </p>
                </div>}
                {notifEnabled && (<>
                  <div style={{borderRadius:14,overflow:"hidden",border:`1px solid ${T.border}`}}>
                    {[["채팅 메시지 알림","새 메시지가 오면 알림"],["파트너 접속 알림","파트너가 앱에 들어오면 알림"],["미션 완료 알림","미션 3개 완료 시 알림"]].map(([title,desc],i)=>(
                      <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",background:T.card,borderBottom:i<2?`1px solid ${T.border}`:"none"}}>
                        <div><p style={{fontWeight:500,fontSize:14,color:T.text}}>{title}</p><p style={{fontSize:12,color:T.sub,marginTop:2}}>{desc}</p></div>
                        <div style={{width:6,height:6,borderRadius:"50%",background:T.p}}/>
                      </div>
                    ))}
                  </div>
                  <p style={{fontSize:12,color:T.sub,marginTop:12,lineHeight:1.6,textAlign:"center"}}>브라우저가 열려있을 때 알림이 와요<br/>앱처럼 홈화면에 추가하면 더 잘 작동해요</p>
                </>)}
              </div>
              {/* PWA 홈화면 추가 가이드 */}
              <div style={{...card,padding:"18px 20px",background:T.l}}>
                <p style={{fontWeight:600,fontSize:14,color:T.text,marginBottom:8}}>📱 앱처럼 사용하기</p>
                <p style={{fontSize:13,color:T.sub,lineHeight:1.7}}>
                  <strong>아이폰:</strong> Safari → 공유 버튼 → "홈 화면에 추가"<br/>
                  <strong>안드로이드:</strong> Chrome → 메뉴 → "홈 화면에 추가"
                </p>
                <p style={{fontSize:12,color:T.p,marginTop:8,fontWeight:500}}>홈화면에 추가하면 앱처럼 전체화면으로 실행돼요!</p>
              </div>
            </div>}

            {settingsTab==="theme" && <div style={{padding:"0 20px"}}>
              <div style={{...card,padding:"20px"}}>
                <p style={{fontSize:11,color:T.sub,fontWeight:600,letterSpacing:".06em",textTransform:"uppercase",marginBottom:14}}>테마 선택</p>
            ...
            {settingsTab==="home" && <div style={{padding:"0 20px"}}>
              <div style={{...card,padding:"20px"}}>
                ...
                <p style={{fontSize:13,color:T.sub,marginBottom:14,lineHeight:1.6}}>배경 사진은 파트너와 실시간으로 공유돼요</p>
                ...
                {missionMode==="custom"&&(<>
                  ...
                  {customMissions.map((m,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:`1px solid ${T.border}`}}>
                      ...
                      <button onClick={()=>{const n=customMissions.filter((_,idx)=>idx!==i);setCustomMissions(n);savePrefs({customMissions:n});}} style={{background:"#FEE8EC",border:"none",borderRadius:8,width:28,height:28,cursor:"pointer",color:"#C45353",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
                    </div>
                  ))}
                </>)}
              </div>
            ...
            {settingsTab==="couple" && couple && <div style={{padding:"0 20px"}}>
              <div style={{...card,padding:"20px"}}>
                <p style={{fontSize:11,color:T.sub,fontWeight:600,letterSpacing:".06em",textTransform:"uppercase",marginBottom:14}}>커플 정보</p>
      ...
      <div style={{position:"absolute",bottom:0,left:0,right:0,background:T.nav,borderTop:`1px solid ${T.border}`,display:"flex",justifyContent:"space-around",paddingTop:8,paddingBottom:"max(12px,env(safe-area-inset-bottom))",zIndex:50,boxShadow:`0 -1px 0 ${T.border}`}}>
        {TABS.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,background:"none",border:"none",cursor:"pointer",padding:"4px 6px",minWidth:44,position:"relative"}}>
            <span style={{fontSize:t.id===tab?24:22,transition:"all .2s",filter:tab===t.id?"none":"grayscale(.2) opacity(.7)"}} className={tab===t.id?"tab-active":""}>{t.icon}</span>
            ...
            {/* Unread badge */}
            {t.id==="chat"&&unread>0&&<div style={{position:"absolute",top:0,right:6,width:16,height:16,borderRadius:"50%",background:"#E05050",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:"#fff",fontWeight:700}}>{unread}</div>}
          </button>
        ))}
      </div>
      ...
      {modal&&(
        <div onClick={()=>setModal(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.46)",zIndex:200,display:"flex",alignItems:modal?.type==="imgView"?"center":"flex-end",justifyContent:"center",backdropFilter:"blur(6px)"}}>
          ...
          {modal?.type!=="imgView"&&(
          <div onClick={e=>e.stopPropagation()} style={{background:T.card,borderRadius:"28px 28px 0 0",padding:"20px 24px",width:"100%",maxWidth:430,maxHeight:"90vh",overflowY:"auto",animation:"scaleIn .4s cubic-bezier(.16,1,.3,1)",paddingBottom:"max(24px,env(safe-area-inset-bottom))"}}>
              ...
              {(events[dk(selDay)]||[]).length>0&&(<>
                ...
                {(events[dk(selDay)]||[]).map((ev,i)=>(
                  <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:`1px solid ${T.border}`}}>
                    ...
                    {ev.who===user.uid&&(<>
                      <button onClick={()=>{setEditEvent(ev);setEventForm({title:ev.title,color:ev.color});setModal("addEvent");}} style={{background:T.l,border:"none",borderRadius:8,width:30,height:30,cursor:"pointer",color:T.p,display:"flex",alignItems:"center",justifyContent:"center"}}>✎</button>
                      <button onClick={()=>delEvent(ev)} style={{background:"#FEE8EC",border:"none",borderRadius:8,width:30,height:30,cursor:"pointer",color:"#C45353",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
                    </>)}
                  </div>
                ))}
            ...
            {modal==="addEvent"&&(<>
              <h3 className="serif" style={{fontSize:26,fontWeight:400,color:T.text,marginBottom:18}}>{editEvent?"일정 수정":"일정 추가"}</h3>
              <p style={{fontSize:11,color:T.sub,fontWeight:600,letterSpacing:".06em",textTransform:"uppercase",marginBottom:10}}>색상</p>
              ...
              <p style={{fontSize:11,color:T.sub,fontWeight:600,letterSpacing:".06em",textTransform:"uppercase",marginBottom:10}}>제목</p>
            ...
            {modal==="addDiary"&&(<>
              <h3 className="serif" style={{fontSize:26,fontWeight:400,color:T.text,marginBottom:18}}>{editDiary?"일기 수정":"일기 쓰기"}</h3>
              <p style={{fontSize:11,color:T.sub,fontWeight:600,letterSpacing:".06em",textTransform:"uppercase",marginBottom:10}}>기분</p>
              ...
              <p style={{fontSize:11,color:T.sub,fontWeight:600,letterSpacing:".06em",textTransform:"uppercase",marginBottom:10}}>스티커</p>
            ...
            {modal==="photoUpload"&&(<>
              <h3 className="serif" style={{fontSize:26,fontWeight:400,color:T.text,marginBottom:18}}>사진 추가</h3>
              <p style={{fontSize:11,color:T.sub,fontWeight:600,letterSpacing:".06em",textTransform:"uppercase",marginBottom:8}}>날짜</p>
              <input type="date" style={{...inp,marginBottom:16}} value={photoDate} onChange={e=>setPhotoDate(e.target.value)}/>
              <p style={{fontSize:11,color:T.sub,fontWeight:600,letterSpacing:".06em",textTransform:"uppercase",marginBottom:8}}>앨범</p>
            ...
            {modal==="addAlbum"&&(<>
              <h3 className="serif" style={{fontSize:26,fontWeight:400,color:T.text,marginBottom:18}}>새 폴더 만들기</h3>
            ...
            {modal==="changeName"&&(<>
              <h3 className="serif" style={{fontSize:26,fontWeight:400,color:T.text,marginBottom:18}}>이름 변경</h3>
